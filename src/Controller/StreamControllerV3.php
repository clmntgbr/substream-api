...
use App\CQRS\Controller\BaseController;
use App\CQRS\Service\CommandQueryFactory;
...
#[Route('/api/streams')]
#[IsGranted('ROLE_USER')]
class StreamControllerV3 extends BaseController
{
    public function __construct(
        protected $commandBus,
        protected $queryBus,
        protected $validator,
        private CommandQueryFactory $commandQueryFactory
    ) {
        parent::__construct($commandBus, $queryBus, $validator);
    }
    ...
    public function createStream(Request $request): JsonResponse
    {
        ...
        // Création de la commande via la factory
        $command = $this->commandQueryFactory->createStreamCommand(
            $data['fileName'],
            ...
            $data['url'],
            $this->getUser(),
            $this->commandQueryFactory->createOptionsFromData($data['options'] ?? [])
        );
    ...
    public function getStream(string $id): JsonResponse
    {
        ...
        // Création de la requête via la factory
        $query = $this->commandQueryFactory->getStreamQuery($streamId);
